...
[source,clojure]
----
    :gen-doc-tests {:extra-deps {com.github.lread/test-doc-blocks {:mvn/version "1.0.116-alpha"}}
...
TIP: Windows users, trying correctly escaping -X args on the command line can lead to frustration.
Consider specifying options in `deps.edn` instead.
