...
If, for whatever reason, you want to rebuild the lint cache supply the `--rebuild-cache` option.
...
* `bb release validate` - to validate your change log is setup appropriately for release
* `bb release prep` - to complete all steps up to, but not including, deploy, note that this will affect changes to `README.adoc`, `CHANGELOG.adoc` and `pom.xml`, you'll NOT want to commit these changes.
